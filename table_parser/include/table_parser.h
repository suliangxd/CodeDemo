...
namespace tp {
     ...
     * 用于解析用户自定义结构的回调函数
     */
    typedef bool (*parser_callback)(const char* s, unsigned len, void* data, unsigned size,
                                    void* context);
    ...
    public:
        ...
        ~TableParser(){ };
    private:
        ParseResult parse_element(unsigned idx, const char* s, unsigned len, void* data);
    private:
        const char* _src;
    ...
    template <typename T>
    unsigned parse_all(const char* src, const ColumnDescriptor desc[], std::vector<T>& out,
